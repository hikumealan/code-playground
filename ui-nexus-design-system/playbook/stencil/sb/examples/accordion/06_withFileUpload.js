...
const addToQueueUnique = (prevQueue, newFiles) => {
  ...
  const merged = [
    ...prevQueue,
    ...newFiles.filter((newFile) => !ids.has(newFile.name))
  ];

  return merged;
};

const renderChip = (files) => files.map((file) => `
  <nexus-chip
...
const accordionFileUpload = document.getElementById('accordion-file-upload');
accordionFileUpload.addEventListener('toggleEvent', () => {
    const open = accordionFileUpload.getAttribute('open') === 'true';
    accordionFileUpload.setAttribute('open', !open);
});

document.getElementById('acc-frmUpload').addEventListener('submit', (event) => {
  handleSubmit(event);
});
document.getElementById('acc-fileupldId').addEventListener('change', (event) => {
  handleChange(event);
});